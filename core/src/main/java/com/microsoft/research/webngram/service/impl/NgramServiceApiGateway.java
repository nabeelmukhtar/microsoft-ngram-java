...
public abstract class NgramServiceApiGateway {
	...
	protected InputStream callApiGet(String apiUrl, int expected) {
		try {
			...
			if (request.getResponseCode() != expected) {
				throw new NgramServiceException(convertStreamToString(request
						.getErrorStream()));
			} else {
			...
			throw new NgramServiceException(e);
		}
	}
	 ...
	 */
	protected InputStream callApiPost(String apiUrl,
			Map<String, List<String>> parameters, int expected) {
		try {
			URL url = new URL(apiUrl);
			HttpURLConnection request = (HttpURLConnection) url
					.openConnection();
			...
			PrintStream out = new PrintStream(new BufferedOutputStream(request
					.getOutputStream()));
	...
	protected String getParametersString(Map<String, List<String>> parameters) {
		...
		for (Iterator<Map.Entry<String, List<String>>> iter1 = parameters
				.entrySet().iterator(); iter1.hasNext();) {
			Map.Entry<String, List<String>> entry = iter1.next();
			for (Iterator<String> iter2 = entry.getValue().iterator(); iter2
					.hasNext();) {
